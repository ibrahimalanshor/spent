...
describe('find one expense categories', function () {
  ...
  it('should found a expense category', (done) => {
    request('http://localhost:5000')
      .get(`/expense-categories/${this.expenseCategory._id}`)
      .expect(200)
      .end((err, res) => {
        if (err) {
          return done(err);
